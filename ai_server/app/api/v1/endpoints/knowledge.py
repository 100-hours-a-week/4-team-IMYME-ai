...
async def refine_candidates_batch(
    request: RefineCandidatesRequest
):
    """
...
async def evaluate_knowledge(
    ...
    if len(request.candidate.text) > 5000:
         return KnowledgeEvaluationResponse(
             ...
             data=KnowledgeEvaluationResult(decision=KnowledgeAction.IGNORE, reasoning="Validation Failed"),
             error={"code": "TEXT_TOO_LONG", "msg": "Candidate text exceeds limit."}
         )

    try:
        result = await knowledge_service.evaluate_knowledge(request.candidate, request.similars)
        return KnowledgeEvaluationResponse(
            success=True,
            data=result,
            error=None
        ...
        return KnowledgeEvaluationResponse(
             success=False,
             data=KnowledgeEvaluationResult(decision=KnowledgeAction.IGNORE, reasoning="LLM Error"),
        ...
        return KnowledgeEvaluationResponse(
             success=False,
             data=KnowledgeEvaluationResult(decision=KnowledgeAction.IGNORE, reasoning="Internal Error"),
             error={"code": "INTERNAL_ERROR", "msg": str(e)}
        )
