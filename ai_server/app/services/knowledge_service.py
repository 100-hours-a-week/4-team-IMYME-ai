...
class KnowledgeService:
    def __init__(self):
        ...
        if settings.GEMINI_API_KEY:
            ...
            logger.warning("GEMINI_API_KEY not found. KnowledgeService may not function correctly.")


    async def refine_candidates_batch(self, items: List[RawFeedbackItem]) -> RefineCandidatesResponseData:
        """
        ...
        async def process_item(item: RawFeedbackItem) -> Optional[KnowledgeCandidate]:
            try:
                ...
                embedding = embedding_service.generate_embedding(refined_text, is_query=False)
    ...
    async def evaluate_knowledge(self, candidate: EvaluateCandidateInput, similars: List[EvaluateSimilarInput]) -> KnowledgeEvaluationResult:
        """
        ...
        try:
             ...
             similars_text_list = []
             for s in similars:
                 similars_text_list.append(f"- ID: {s.id} (Similarity: {s.similarity:.4f})\n  Content: {s.text}")
             ...
             try:
                 ...
                 logger.warning(f"Invalid decision code '{decision_str}', defaulting to IGNORE.")
             ...
             if decision in [KnowledgeAction.UPDATE] and final_content:
                 final_vector = embedding_service.generate_embedding(final_content, is_query=False)
